...
export function MonetizationSettingsForm({ values }: { values: Record<string, unknown> }) {
  ...
  return <form action={action} className="mt-10 space-y-6">
    ...
    <FormSection title="Popunder" description="Usually site-wide. Paste the exact script supplied by your ad network.">
      ...
      <CodeEditor name="popunderCode" label="Popunder code" defaultValue={String(values.popunderCode ?? "")} hint="Production loads this code in the page <head>, matching ad-network instructions. Your browser may still block popups until a user interaction occurs." error={state.fieldErrors?.popunderCode} />
    </FormSection>
